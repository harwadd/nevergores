...
std::string FilterLinks(const std::string &msg)
{
	std::string filtered = msg;

	// Regex para links do Discord (convites)
	std::regex inviteRegex(R"((?:https?:\/\/)?(?:www\.)?(?:discord\.(?:gg|io|me|li)|discordapp\.com\/invite)\/[a-zA-Z0-9]+)");

	// Regex para qualquer link genérico (inclui .xyz, .client, .agency, etc.)
	std::regex urlRegex(R"((?:https?:\/\/)?(?:www\.)?[a-zA-Z0-9\-]+\.[a-zA-Z0-9\-]{2,}(?:\/\S*)?)");

	// Regex para endereços IP (IPv4) com ou sem porta
	std::regex ipRegex(R"((?:https?:\/\/|udp:\/\/)?(?:\d{1,3}\.){3}\d{1,3}(?::\d{1,5})?)");

	// Substitui todos por [LINK]
	filtered = std::regex_replace(filtered, inviteRegex, "[LINK]");
	filtered = std::regex_replace(filtered, urlRegex, "[LINK]");
	filtered = std::regex_replace(filtered, ipRegex, "[IP]");
...
void CGameContext::OnSayNetMessage(const CNetMsg_Cl_Say *pMsg, int ClientId, const CUnpacker *pUnpacker)
{
	...
	else
	{
		...
		std::string FilteredMsg = FilterLinks(Msg);
