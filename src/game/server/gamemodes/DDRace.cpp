...
void CGameControllerDDRace::OnPlayerConnect(CPlayer *pPlayer)
{
	...
	if(!Server()->ClientPrevIngame(ClientId)) {
		char aBuf[512];
		str_format(aBuf, sizeof(aBuf), "✦ Welcome, '%s'.", Server()->ClientName(ClientId), GetTeamName(pPlayer->GetTeam()));
		GameServer()->SendChat(-1, TEAM_ALL, aBuf, -1, CGameContext::FLAG_SIX);
		GameServer()->SendChatTarget(ClientId, "Gores Modded. Version: " GAME_VERSION);
		GameServer()->SendChatTarget(ClientId, "please visit clan.tw or say /info.");
	}

	// send display message on join
...
void CGameControllerDDRace::OnReset()
{
	...
	// ngores
	if(m_pLoadBestTimeResult && m_pLoadBestTimeResult->m_CurrentRecord.has_value())
	{
		str_copy(m_CurrentRecordHolder, m_pLoadBestTimeResult->m_aPlayerName, sizeof(m_CurrentRecordHolder));

		dbg_msg("record_flag", "Recordista carregado: '%s' (tempo %.2f)",
			m_pLoadBestTimeResult->m_aPlayerName, *m_pLoadBestTimeResult->m_CurrentRecord);
	}
	else
	{
		dbg_msg("record_flag", "Nenhum recordista carregado ainda.");
	}
}
...
void CGameControllerDDRace::Tick()
{
	...
	//{
	//dbg_msg("record_flag", "Recordista atual: %s", g_NomeRecordista.c_str());
	//}
}
