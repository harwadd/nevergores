	...
	m_UniqueClientId(UniqueClientId)
{
	...
	// ngores
	m_RainbowSpeed = 1; // Velocidade padr�o
...
void CPlayer::Tick()
{
	//dbg_msg("webhook", "CPlayer::Tick chamado para %d", m_ClientId);
...
void CPlayer::Snap(int SnappingClient)
{
	...
	if(m_pGameServer->Server()->GetAuthedState(m_ClientId) >= AUTHED_MOD) // apenas para MOD+
	{
		...
		m_Powers.m_HasRainbowBlack = true;
	...
	{
		pClientInfo->m_UseCustomColor = true;
		pClientInfo->m_ColorBody = m_PowersData.m_RainbowColorLight;
		pClientInfo->m_ColorFeet = m_PowersData.m_RainbowColorLight;
	}
	else
	{
		pClientInfo->m_UseCustomColor = m_TeeInfos.m_UseCustomColor;
