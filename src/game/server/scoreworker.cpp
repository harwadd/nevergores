...
#include "gamecontroller.h" // <-- precisa estar no topo
...
bool CScoreWorker::Init(IDbConnection *pSqlServer, const ISqlData *pGameData, char *pError, int ErrorSize)
{
    ...
    if(!pResult)
    {
        dbg_msg("record_flag", "Erro: pResult é nulo em Init()");
        return true;
    }

    char aMap[64];
    str_copy(aMap, g_Config.m_SvMap, sizeof(aMap));
    ...
    if(!End)
    {
        ...
        str_copy(pResult->m_CurrentRecordHolder, aBuf2, sizeof(pResult->m_CurrentRecordHolder));

        dbg_msg("record_flag", "Recordista encontrado: '%s' (tempo %.2f)", pResult->m_CurrentRecordHolder, pResult->m_CurrentRecord);
    }
    else
    {	
		dbg_msg("record_flag", "Isso é apenas um teste");
        dbg_msg("record_flag", "Nenhum recordista encontrado para o mapa %s", aMap);
    }
...
bool CScoreWorker::LoadBestTime(IDbConnection *pSqlServer, const ISqlData *pGameData, char *pError, int ErrorSize)
{
	...
	if(!pData || !pResult)
	{
		str_copy(pError, "LoadBestTime: invalid data/result pointer", ErrorSize);
		return false;
	}
...
if(!End)
{
	...
	dbg_msg("record_flag", "Melhor tempo do mapa '%s' é %.2f por '%s'",
		pData->m_aMap, *pResult->m_CurrentRecord, pResult->m_aPlayerName);
		
	g_NomeRecordista = pResult->m_aPlayerName;
	g_MelhorTempo = *pResult->m_CurrentRecord;


	pResult->m_Completed = true; // ✅ marca que o resultado está pronto
	}
else
{
	dbg_msg("record_flag", "Nenhum recorde encontrado para o mapa '%s'", pData->m_aMap);
	pResult->m_Completed = true; // ainda assim marca como pronto (mesmo sem recorde)
}
