...
void IGameController::SetRecordFlagForPlayer(CPlayer *pPlayer)
{
    if(!pPlayer || !pPlayer->GetCharacter())
        return;

    int CID = pPlayer->GetCid();
    const char *pName = Server()->ClientName(CID);

    if(!pName || pName[0] == '\0')
    {
        dbg_msg("record_flag", "Tentativa de setar recordista falhou: nome inválido (CID %d)", CID);
        return;
    }

    // Copia o nome para o record holder
    str_copy(m_CurrentRecordHolder, pName, sizeof(m_CurrentRecordHolder));

    // Atualiza a referência para o personagem do recordista
    UpdateRecordFlag();

    dbg_msg("record_flag", "Recordista atualizado: '%s' (CID %d)", m_CurrentRecordHolder, CID);
}
...
int IGameController::SnapRecordFlag(int SnappingClient)
{
    ...
    // Team visibility (não aplica para demo client)
