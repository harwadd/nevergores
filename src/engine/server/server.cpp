...
void CServer::RegisterCommands()
{
	...
	// Inicializa a whitelist com console e storage
	g_Whitelist.Init(m_pConsole, m_pStorage);

	// ngores
	Console()->Register("rainbow", "i[id] s[speed]", CFGFLAG_SERVER, ConRainbow, this, "Set rainbow mode");
	Console()->Register("trail", "i[id]", CFGFLAG_SERVER, ConTrail, this, "Set trail");
	Console()->Register("stars", "?i", CFGFLAG_SERVER, ConStars, this, "Toggle stars trail (optional player id)");
	Console()->Register("aurashotgun", "?i", CFGFLAG_SERVER, ConAuraShotgun, this, "Toggle aura shotgun (optional player id)");
	Console()->Register("auragun", "?i", CFGFLAG_SERVER, ConAuraGun, this, "Toggle aura gun (optional player id)");
	Console()->Register("auradot", "?i", CFGFLAG_SERVER, ConAuraDot, this, "Toggle aura dot (optional player id)");
	Console()->Register("splash", "?i", CFGFLAG_SERVER, ConSplash, this, "Toggle splash effect (optional player id)");
	Console()->Register("explosion", "?i", CFGFLAG_SERVER, ConExplosion, this, "Toggle explosion effect (optional player id)");

	Console()->Register("guided_heart", "", CFGFLAG_SERVER, ConGuidedHeart, this, "Send an guided heart");
	Console()->Register("guided_shield", "", CFGFLAG_SERVER, ConGuidedShield, this, "Send an guided shield");
	Console()->Register("guided_ninja", "", CFGFLAG_SERVER, ConGuidedNinjaSword, this, "Send an guided ninja");
...
void CServer::ConRainbow(IConsole::IResult *pResult, void *pUserData)
{
	...
	if(pSelf->GetAuthedState(pResult->m_ClientId) < AUTHED_MOD)
	{
		pGameCtx->SendChatTarget(pResult->m_ClientId, "Você não tem permissão.");
		return;
	}

	if(pResult->NumArguments() < 2)
	{
		pGameCtx->SendChatTarget(pResult->m_ClientId, "Uso: rainbow <id> <velocidade>");
		return;
	}

	int TargetID = pResult->GetInteger(0);
	if(TargetID < 0 || TargetID >= MAX_CLIENTS || !pGameCtx->m_apPlayers[TargetID])
	{
		pGameCtx->SendChatTarget(pResult->m_ClientId, "ID inválido.");
		return;
	}
	...
	{
		pGameCtx->SendChatTarget(pResult->m_ClientId, "Use velocidade entre 0 e 100");
		return;
	}
	...
	if(Speed == 0)
	{
		...
		pPlayer->m_Powers.m_HasRainbowBlackEnabled = false;

		//pGameCtx->SendChatTarget(pResult->m_ClientId, "❌ Rainbow desativado!");
		return;
	}
	...
	//pGameCtx->SendChatTarget(pResult->m_ClientId, "🌈 Rainbow ativado!");
}
...
void CServer::ConTrail(IConsole::IResult *pResult, void *pUserData)
{
    ...
    if(pSelf->GetAuthedState(ClientID) < AUTHED_MOD)
    {
        pGameServer->SendChatTarget(ClientID, "You do not have permission to use this command.");
        return;
    }
    ...
    if(pResult->NumArguments() >= 1)
    {
        int ArgID = pResult->GetInteger(0);
        if(ArgID < 0 || ArgID >= MAX_CLIENTS || !pGameServer->m_apPlayers[ArgID])
        {
            pGameServer->SendChatTarget(ClientID, "Invalid player ID.");
            return;
        }
        TargetID = ArgID;
    }
    ...
    if(pPlayer->m_Powers.m_HasTrailEnabled) {
	}
        //pGameServer->SendChatTarget(ClientID, "✨ Trail enabled!");
    else {
        //pGameServer->SendChatTarget(ClientID, "❌ Trail disabled!");
	}
}
...
void CServer::ConStars(IConsole::IResult *pResult, void *pUserData)
{
	...
	if(pPlayer->m_Powers.m_HasStarEnabled) {
	}
		//pGameServer->SendChatTarget(ClientID, "✨ Stars enabled!");
	else {
		//pGameServer->SendChatTarget(ClientID, "❌ Stars disabled!");
	}
}

void CServer::ConAuraShotgun(IConsole::IResult*, void*) {

}

void CServer::ConAuraGun(IConsole::IResult*, void*) {

}

void CServer::ConAuraDot(IConsole::IResult*, void*) {

}

void CServer::ConSplash(IConsole::IResult*, void*) {


}

void CServer::ConExplosion(IConsole::IResult*, void*) {
...
{
	// Conversões corretas do contexto
	CServer *pSelf = (CServer *)pUserData;
	IGameServer *pGameServer = pSelf->GameServer();
	CGameContext *pGameCtx = (CGameContext *)pGameServer;
	...
	// Verifica permissão
	if(pGameCtx->Server()->GetAuthedState(ClientID) < AUTHED_MOD)
	{
		pGameCtx->SendChatTarget(ClientID, "You do not have permission to use this command.");
		return;
	}
	...
	// Executa o drop guiado (procura o jogador mais próximo)
	pGameCtx->ExecuteDrop(pResult, pGameCtx, EMOTICON_HEARTS, EMOTE_HAPPY, POWERUP_HEALTH, true);
}
...
void CServer::ConGuidedShield(IConsole::IResult *pResult, void *pUserData)
{
	...
	pGameCtx->ExecuteDrop(pResult, pGameCtx, EMOTICON_EYES, EMOTE_HAPPY, POWERUP_ARMOR, true);
}
...
void CServer::ConGuidedNinjaSword(IConsole::IResult *pResult, void *pUserData)
{
	...
	pGameCtx->ExecuteDrop(pResult, pGameCtx, EMOTICON_GHOST, EMOTE_SURPRISE, POWERUP_NINJA, true);
}