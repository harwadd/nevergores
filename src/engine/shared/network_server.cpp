...
void SendDiscordWebhookAsync(const char *pWebhookUrl, const char *pMessage)
{
	std::string url = pWebhookUrl;
...
{
	// Verifica whitelist antes de aceitar o cliente
	if(g_Config.m_SvWhitelist && !g_Whitelist.IsWhitelisted(&Addr))
	{
		// Log to console for debugging
		char aAddrStr[NETADDR_MAXSTRSIZE];
		net_addr_str(&Addr, aAddrStr, sizeof(aAddrStr), false);
		dbg_msg("whitelist", "Player from IP %s tried to connect but is not whitelisted.", aAddrStr);

		// Send webhook notification asynchronously
		char aWebhookMsg[256];
		str_copy(aWebhookMsg, "Player 'Unknown' from IP ", sizeof(aWebhookMsg));
		str_append(aWebhookMsg, aAddrStr, sizeof(aWebhookMsg));
		str_append(aWebhookMsg, " tried to connect but is not whitelisted.", sizeof(aWebhookMsg));
		const char *webhook_url = "WEBHOOK_HERE";
		SendDiscordWebhookAsync(webhook_url, aWebhookMsg);

		const char aMsg[] = "server shutdown";
		CNetBase::SendControlMsg(m_Socket, &Addr, 0, NET_CTRLMSG_CLOSE, aMsg, sizeof(aMsg), SecurityToken, Sixup);
		return -1; // rejeita o cliente sem banir
	}
